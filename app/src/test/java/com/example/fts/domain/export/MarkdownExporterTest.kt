...
class MarkdownExporterTest {
    private fun snapshot(): Snapshot {
        ...
        return Snapshot(rootName = "root", rootUri = "content://root", scannedAt = 100L, root = root, stats = SnapshotStats(3, 2, 1, 42))
    }

    @Test fun allFourModelsFollowTheirRequiredShape() {
        val s = snapshot()
        ...
        assertTrue(a.contains("## folder")); assertTrue(a.contains("- File\\_\\[one\\].md"))
        assertTrue(b.contains("## folder")); assertTrue(b.contains("---"))
        assertTrue(c.contains("- root/")); assertTrue(c.contains("  - folder/"))
        assertTrue(d.contains("root/")); assertTrue(d.contains("└── folder/")); assertTrue(d.contains("    └── File\\_\\[one\\].md"))
    }

    @Test fun markdownEscapesOnlySpecifiedSpecialCharacters() {
        val s = snapshot().copy(rootName = "root_[x]*-!.md")
        val md = MarkdownExporter.export(s, MarkdownModel.D)
        assertTrue(md.contains("root\\_\\[x\\]\\* -!" ) || md.contains("root\\_\\[x\\]\\*-!.md"))
    }
}
