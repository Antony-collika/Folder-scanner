...
object TreeFlattener {
    ...
    private fun flattenRecursive(entry: Entry, depth: Int, expanded: Set<String>, result: MutableList<FlatNode>) {
        ...
        if (isExpanded && hasChildren) {
            entry.children!!.forEach { flattenRecursive(it, depth + 1, expanded, result) }
        }
    }
}