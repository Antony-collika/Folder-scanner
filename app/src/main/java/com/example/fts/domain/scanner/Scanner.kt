...
class Scanner(private val context: Context, private val cacheManager: CacheManager) {
    ...
    private suspend fun calculateStats(entry: Entry): SnapshotStats { var totalEntries = 0; var totalFolders = 0; var totalFiles = 0; var totalSize = 0L; val stack = ArrayDeque<Entry>(); stack.add(entry); while (stack.isNotEmpty()) { coroutineContext.ensureActive(); val current = stack.removeLast(); totalEntries++; when (current.type) { EntryType.FOLDER -> totalFolders++; EntryType.FILE -> { totalFiles++; totalSize += current.size ?: 0L } }; current.children?.forEach(stack::add) }; return SnapshotStats(totalEntries, totalFolders, totalFiles, totalSize) }
    suspend fun estimateCount(rootUri: Uri): Int = withContext(Dispatchers.IO) { try { val rootDoc = SafDocument.fromTreeUri(context, rootUri) ?: return@withContext 0; var count = 0; val stack = ArrayDeque<SafDocument>(); stack.add(rootDoc); while (stack.isNotEmpty() && count < 100001) { coroutineContext.ensureActive(); val doc = stack.removeLast(); count++; if (doc.isDirectory) doc.listFiles()?.forEach(stack::add) }; count } catch (_: Exception) { 0 } }
}