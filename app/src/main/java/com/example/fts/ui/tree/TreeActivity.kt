...
class TreeActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (rootUri == null) {
            Toast.makeText(this, "Không có thư mục được chọn", Toast.LENGTH_SHORT).show()
            finish()
            return
        }
        ...
        adapter = TreeAdapter(
            viewModel.markdownModel(),
            viewModel.showMetadataInMarkdown(),
        ...
        viewModel.treeItems.observe(this) {
        ...
        viewModel.exportResult.observe(this) { result ->
            result.onSuccess { Toast.makeText(this, "Đã xuất thành công", Toast.LENGTH_SHORT).show() }
                .onFailure { Toast.makeText(this, "Xuất thất bại: ${it.message}", Toast.LENGTH_SHORT).show() }
        }
        viewModel.isLoading.observe(this) { loading ->
            findViewById<Button>(R.id.exportButton).isEnabled = !loading
            findViewById<Button>(R.id.exportJsonButton).isEnabled = !loading
    ...
    private fun renderLevelControls() {
        ...
        for (level in 1..maxLevel) {
            val icon = TextView(this).apply {
                ...
                isClickable = true
                isFocusable = true
                alpha = if (level == selected) 1f else 0.7f
                contentDescription = "Hiển thị đến cấp $level"
                setOnClickListener { viewModel.setExpandLevel(level) }
            }
    ...
    private fun showSortDialog() {
        val labels = arrayOf("Tên (folder trước, A-Z)", "Ngày sửa (mới nhất)", "Kích thước (lớn nhất)", "Loại (folder trước)")
        val values = arrayOf("name", "date", "size", "type")
        val current = values.indexOf(viewModel.sortOrder()).coerceAtLeast(0)
        AlertDialog.Builder(this).setTitle("Sắp xếp cây").setSingleChoiceItems(labels, current) { dialog, which -> viewModel.setSortOrder(values[which]); dialog.dismiss() }.show()
    }

    private fun exportMarkdown() {
        currentMarkdown()?.let {
            viewModel.prepareExport(it, "md")
    ...
    private fun currentMarkdown(): String? = viewModel.snapshot.value?.let {
        MarkdownExporter.export(it, viewModel.markdownModel(), viewModel.showMetadataInMarkdown(), viewModel.sortOrder())
    }
    ...
    private fun showFileDetail(entry: Entry) {
        ...
        AlertDialog.Builder(this).setTitle("Chi tiết file").setMessage(details).setPositiveButton("Copy đường dẫn") { _, _ -> copyPath(entry.path) }.setNegativeButton("Đóng", null).show()
    }

    private fun copyPath(path: String) {
        (getSystemService(Context.CLIPBOARD_SERVICE) as ClipboardManager).setPrimaryClip(ClipData.newPlainText("Path", path))
        Toast.makeText(this, "Đã copy đường dẫn", Toast.LENGTH_SHORT).show()
    }

    private fun openDiff(rootUri: String, displayName: String) {
        startActivity(Intent(this, DiffActivity::class.java).apply { putExtra(EXTRA_ROOT_URI, rootUri); putExtra(EXTRA_DISPLAY_NAME, displayName) })
    }
