...
class TreeActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        viewModel = TreeViewModel(rootUri, displayName, application); adapter = TreeAdapter({ viewModel.toggleFolder(it) }, { showFileDetail(it) }, { copyPath(it.path) }); recyclerView.layoutManager = LinearLayoutManager(this); recyclerView.adapter = adapter
        viewModel.treeItems.observe(this) { adapter.submitList(it) }; viewModel.snapshot.observe(this) { updateHeader(it) }; viewModel.showDiff.observe(this) { if (it) startActivity(Intent(this, DiffActivity::class.java).apply { putExtra(EXTRA_ROOT_URI, rootUri); putExtra(EXTRA_DISPLAY_NAME, displayName) }) }; viewModel.exportResult.observe(this) { result -> result.onSuccess { Toast.makeText(this, "Đã xuất thành công", Toast.LENGTH_SHORT).show() }.onFailure { e -> Toast.makeText(this, "Xuất thất bại: ${e.message}", Toast.LENGTH_SHORT).show() } }
        searchInput.addTextChangedListener(object : TextWatcher { override fun beforeTextChanged(s: CharSequence?, start: Int, count: Int, after: Int) = Unit; override fun onTextChanged(s: CharSequence?, start: Int, before: Int, count: Int) { viewModel.search(s?.toString().orEmpty()) }; override fun afterTextChanged(s: Editable?) = Unit }); viewModel.loadData()
    }
