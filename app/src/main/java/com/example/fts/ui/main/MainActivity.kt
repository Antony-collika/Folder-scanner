...
class MainActivity : AppCompatActivity() {
    ...
    private val createDocumentLauncher = registerForActivityResult(ActivityResultContracts.CreateDocument()) { uri ->
        ...
        pendingExport = null
        if (uri != null && content != null) lifecycleScope.launch(Dispatchers.IO) {
            runCatching { contentResolver.openOutputStream(uri)?.use { it.write(content.toByteArray(Charsets.UTF_8)) } ?: error("Không thể mở tệp đích") }
                .onSuccess { runOnUiThread { Toast.makeText(this@MainActivity, "Đã xuất thành công", Toast.LENGTH_SHORT).show() } }
                .onFailure { runOnUiThread { Toast.makeText(this@MainActivity, "Xuất thất bại: ${it.message}", Toast.LENGTH_LONG).show() } }
        }
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (Build.VERSION.SDK_INT >= 33 && ContextCompat.checkSelfPermission(this, Manifest.permission.POST_NOTIFICATIONS) != PackageManager.PERMISSION_GRANTED) requestPermissions(arrayOf(Manifest.permission.POST_NOTIFICATIONS), 100)
    ...
    override fun onResume() {
        ...
        if (pendingPermissionRequest) {
            if (BroadStorageAccess.hasAccess(this)) showFilesystemPicker(Environment.getExternalStorageDirectory())
    ...
    private fun showRootActions(root: RootFolder) {
        ...
        AlertDialog.Builder(this).setTitle(root.displayName).setItems(actions) { _, which -> when (which) {
            0 -> openTree(root); 1 -> exportRoot(root, false); 2 -> exportRoot(root, true); 3 -> startScanService(root.uri, root.displayName, true); 4 -> confirmRemoveRoot(root)
        } }.show()
    }

    private fun openTree(root: RootFolder) { startActivity(Intent(this, TreeActivity::class.java).apply { putExtra(TreeActivity.EXTRA_ROOT_URI, root.uri); putExtra(TreeActivity.EXTRA_DISPLAY_NAME, root.displayName) }) }

    private fun exportRoot(root: RootFolder, json: Boolean) {
        lifecycleScope.launch {
            val snapshot = withContext(Dispatchers.IO) { cacheManager.load(root.uri) }
            if (snapshot == null) { Toast.makeText(this@MainActivity, "Chưa có snapshot. Hãy quét thư mục trước.", Toast.LENGTH_SHORT).show(); return@launch }
            val content = if (json) JsonExporter.export(snapshot) else MarkdownExporter.export(snapshot, settingsRepository.markdownModel(), settingsRepository.showMetadataInMarkdown(), settingsRepository.sortOrder())
            pendingExport = content
            createDocumentLauncher.launch("tree_${root.displayName}_${System.currentTimeMillis()}.${if (json) "json" else "md"}")
        }
    }

    private fun confirmRemoveRoot(root: RootFolder) { AlertDialog.Builder(this).setTitle("Xóa khỏi danh sách?").setMessage("Thư mục sẽ được bỏ khỏi màn hình chính. Snapshot/cache vẫn được giữ lại để có thể dùng khi thêm lại và quét lại sau này.").setNegativeButton("Hủy", null).setPositiveButton("Xóa") { _, _ -> viewModel.deleteRootFolder(root.uri) }.show() }

    private fun chooseRoot() {
        if (BroadStorageAccess.hasAccess(this)) { showFilesystemPicker(Environment.getExternalStorageDirectory()); return }
        AlertDialog.Builder(this)
            ...
            .setPositiveButton("Cho phép quyền") { _, _ -> pendingPermissionRequest = true; BroadStorageAccess.request(this) }
            .show()
    }
    ...
    private fun showFilesystemPicker(start: File) {
        ...
        fun render() {
            ...
            AlertDialog.Builder(this).setTitle("Chọn thư mục\n${current.absolutePath}").setItems(children.map { it.name }.toTypedArray()) { _, which -> current = children[which]; render() }
                ...
                .setNeutralButton("Chọn thư mục này") { _, _ -> onFolderSelected(Uri.fromFile(current), current.name.ifBlank { "Internal storage" }, "Filesystem") }.show()
        }
        render()
    }
    ...
    private fun onFolderSelected(uri: Uri, displayName: String, source: String) {
        ...
        lifecycleScope.launch {
            ...
            AlertDialog.Builder(this@MainActivity).setTitle("Xác nhận quét").setMessage("Thư mục: ${rootFolder.displayName}\nNguồn: $source\nƯớc tính: ~$estimateText\n\nQuét bằng $source.").setNegativeButton("Để sau", null).setPositiveButton("Bắt đầu quét") { _, _ -> startScanService(rootFolder.uri, rootFolder.displayName, false) }.show()
        }
    }
