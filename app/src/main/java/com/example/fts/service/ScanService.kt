...
class ScanService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        isCancelled = false
        ScanProgressBus.update(rootUri, 0)
        startForeground(Constants.NOTIFICATION_ID_SCAN, createNotification("Đang chuẩn bị quét...", true))
        scanJob?.cancel()
        scanJob = serviceScope.launch {
            val startedAt = System.currentTimeMillis()
            try {
                val previous = if (isIncremental) cacheManager.load(rootUri) else null
                val snapshot = scanner.scan(Uri.parse(rootUri), rootName, settingsRepository.scanOptions()) { progress ->
                    if (isActive && !isCancelled) {
                        ScanProgressBus.update(rootUri, progress.count)
                ...
                rootFolderRepository.getByUri(rootUri)?.let {
                    rootFolderRepository.update(it.copy(lastScannedAt = snapshot.scannedAt, entryCount = snapshot.stats.totalEntries))
                }
                ScanProgressBus.finish(rootUri)
                if (settingsRepository.notificationOnScanComplete()) {
                    showCompleteNotification(rootName, snapshot.stats.totalEntries, diff, rootUri, rootName, (System.currentTimeMillis() - startedAt) / 1000)
                }
    ...
    private fun updateNotification(message: String) {
        (getSystemService(NOTIFICATION_SERVICE) as NotificationManager).notify(Constants.NOTIFICATION_ID_SCAN, createNotification(message, true))
    }
    ...
    private fun showCompleteNotification(rootName: String, totalEntries: Int, diff: Diff?, rootUri: String, displayName: String, durationSeconds: Long) {
        ...
        val intent = (if (changeCount != null) Intent(this, DiffActivity::class.java) else Intent(this, TreeActivity::class.java)).apply {
            putExtra(Constants.EXTRA_URI, rootUri)
            putExtra(Constants.EXTRA_DISPLAY_NAME, displayName)
            putExtra(TreeActivity.EXTRA_ROOT_URI, rootUri)
            putExtra(TreeActivity.EXTRA_DISPLAY_NAME, displayName)
        }
        val pending = PendingIntent.getActivity(this, 1, intent, PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE)
        (getSystemService(NOTIFICATION_SERVICE) as NotificationManager).notify(Constants.NOTIFICATION_ID_SCAN_COMPLETE,
            NotificationCompat.Builder(this, Constants.NOTIFICATION_CHANNEL_ID)
                .setContentTitle("Quét hoàn tất").setContentText(message).setSmallIcon(R.drawable.ic_folder).setContentIntent(pending).setAutoCancel(true).build())
    }

    private fun showCancelledNotification(rootName: String) = (getSystemService(NOTIFICATION_SERVICE) as NotificationManager).notify(Constants.NOTIFICATION_ID_SCAN_COMPLETE,
        NotificationCompat.Builder(this, Constants.NOTIFICATION_CHANNEL_ID).setContentTitle("Đã hủy quét").setContentText("Đã hủy quét $rootName").setSmallIcon(R.drawable.ic_folder).setAutoCancel(true).build())

    private fun showErrorNotification(rootName: String, error: String) = (getSystemService(NOTIFICATION_SERVICE) as NotificationManager).notify(Constants.NOTIFICATION_ID_SCAN_COMPLETE,
        NotificationCompat.Builder(this, Constants.NOTIFICATION_CHANNEL_ID).setContentTitle("Lỗi quét").setContentText("Không thể quét $rootName: $error").setSmallIcon(R.drawable.ic_folder).setAutoCancel(true).build())
}
