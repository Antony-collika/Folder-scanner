...
class SafTreeReader(private val context: Context) {
    suspend fun readTree(rootUri: Uri, options: ScanOptions, previousRoot: Entry? = null, onProgress: (ScanProgress) -> Unit): Entry {
        ...
        while (stack.isNotEmpty()) {
            val (doc, entry) = stack.removeLast(); val oldChildren = oldStack.removeLast(); val children = listFilesWithRetry(doc, options)
            for (child in children) {
                val id = getDocumentId(child) ?: ""; val old = oldChildren[id]; val type = if (child.isDirectory) EntryType.FOLDER else EntryType.FILE; val childPath = buildPath(entry.path, child.name ?: "")
    ...
    private fun listFilesWithRetry(doc: SafDocument, options: ScanOptions): List<SafDocument> {
        repeat(2) { attempt ->
            try { return doc.listFiles().asSequence().filter { passesFilter(it, options) }.sortedWith(comparator(options.sortOrder)).toList() }
            catch (_: Exception) { if (attempt == 1) return emptyList() }
        }
        return emptyList()
    }
