...
class RootFolderRepository(private val context: Context) {
    ...
    fun getByUri(uri: String): RootFolder? {
        return getAll().find { it.uri == uri }
    }
    
    fun add(rootFolder: RootFolder): Boolean {
        val currentList = getAll().toMutableList()
        
        if (currentList.any { it.uri == rootFolder.uri }) {
            return false
        }
        
        if (currentList.size >= MAX_FOLDERS) {
            return false
        }
    ...
    private fun saveList(list: List<RootFolder>) {
        ...
        sharedPrefs.edit().putString(KEY_ROOT_FOLDERS, jsonString).apply()
    }
}